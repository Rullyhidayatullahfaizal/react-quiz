...
export const Question = ({setDisplay, setScore, index, answer, handleNext, handlePrev, setAnswer}) => {
    ...
    <div className='h-full relative p-[54px] font-alfa flex flex-col gap-[54px]'>
			<>
				...
				<div className='grid grid-cols-2 gap-x-[142px] gap-y-[42px] mx-[20px]'>
					{Object.entries(questions[index].options).map(([key, val]) => {
						return (
							<div key={key}  onClick={() => handleAns(key,index+1)} className={` h-[111px] rounded-[20px] text-[20px] flex items-center text-white p-4 cursor-pointer hover:bg-chocolate hover:border-solid hover:border-4 hover:border-white ${answer[index+1] === key? 'bg-chocolate border-solid border-4 border-white': 'bg-blue'}`}>
								{`${key}. ${val}`}
							</div>
						);
					})}
			...
			<div className='absolute bottom-[60px] right-[54px] flex justify-end mx-[20px] gap-5 '>
				{index !== 0 && <svg
					...
					height='73'
					viewBox='0 0 72 73'
					className='bg-chocolate rounded-full border-solid border-[6px] border-blue hover:border-white hover:scale-110 hover:shadow-2xl hover:fill-white '
					fill='#EAB2A0'>
				...
				{index+1 === questions.length &&<div onClick={handleSubmit} className='text-[36px] bg-chocolate flex items-center p-1 text-blue border-[6px] border-blue hover:scale-110 hover:text-white hover:border-white'>
					Finish
				</div>}
			</div>
		</div>
